...
- 本地导入整理：支持 Web 上传、iOS 快捷指令上传和 `/app/local` 文件夹监听，自动解析、去重、合并配置并生成本地模型。
...
- `/app/config`：运行配置和旧配置迁移输入。
- `/app/logs`：历史日志迁移输入和可选日志镜像。
- `/app/state`：旧任务状态、marker、系统状态迁移输入和少量临时状态。
- `/app/archive`：归档模型、图片、附件、模型文件和历史 `meta.json`。
- `/app/local`：本地导入和整理入口。
...
```env
MAKERHUB_POSTGRES_PASSWORD=change-this-password
```
...
docker compose up -d makerhub-postgres makerhub-app makerhub-worker
```

如果设置页提示“需改 compose”，说明当前容器缺少 `MAKERHUB_DATABASE_URL` 或 `makerhub-postgres`。请先升级 compose，再使用网页一键更新。
...
- 默认部署升级为 App / Worker / Postgres 三容器，Compose 增加 `makerhub-postgres`、`MAKERHUB_DATABASE_URL` 和数据库健康检查。
