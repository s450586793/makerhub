...
_BROWSER_CONTROL_LOCKS: dict[str, threading.RLock] = {}
_BROWSER_CONTROL_LOCKS_GUARD = threading.Lock()
...
def _browser_control_lock(url: str) -> threading.RLock:
    host = (urlparse(url).netloc or "makerworld.com.cn").lower()
    platform = "global" if host.endswith(".com") else "cn"
    with _BROWSER_CONTROL_LOCKS_GUARD:
...
def _fetch_listing_html(session: requests.Session, page_url: str, raw_cookie: str) -> str:
    ...
    headers = {
        ...
        "Sec-Fetch-Site": "none",
        "Sec-Fetch-User": "?1",
        "User-Agent": session.headers.get("User-Agent", BROWSER_USER_AGENT),
    }
    if cookie_header:
        headers["Cookie"] = cookie_header
    with _browser_control_lock(page_url):
        html = makerworld_browser_get_text(
            page_url,
...
def _api_get_json(
    ...
    *,
    ...
    for api_url in _service_endpoint_candidates(source_url, service_name, path):
        started = time.time()
        try:
            with _browser_control_lock(source_url):
...
def discover_account_profile(platform: str, raw_cookie: str) -> dict[str, Any]:
    ...
    session.cookies.update(parse_cookie_values(raw_cookie))
...
def discover_account_home_summary(
    platform: str,
    raw_cookie: str,
...
def discover_followed_authors_from_page(
    platform: str,
    raw_cookie: str,
    profile: dict[str, str],
) -> dict[str, Any]:
...
def discover_followed_collections(
    platform: str,
    raw_cookie: str,
    *,
    uid: str = "",
...
def default_favorites_source(platform: str, profile: dict[str, str]) -> dict[str, str]:
    clean_platform = "global" if str(platform or "").strip().lower() == "global" else "cn"
    handle = _profile_handle_or_uid_handle(profile)
    if not handle:
        return {}
...
def _resolve_author_uid_from_sample_models(
    ...
    for model_url in page_links[:8]:
        design = _fetch_sample_design(session, source_url, raw_cookie, model_url)
...
def resolve_source_name(url: str, raw_cookie: str) -> str:
...
def discover_source(url: str, raw_cookie: str, max_pages: int = 12) -> dict:
    source_url = normalize_source_url(url)
    session = requests.Session()
    session.headers.update({"User-Agent": BROWSER_USER_AGENT})
    session.cookies.update(parse_cookie_values(raw_cookie))
