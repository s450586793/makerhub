...
- `makerhub-worker`：归档队列、订阅同步、源端刷新、本地整理、数据库迁移、Three.js 封面生成等后台任务。
...
- Postgres 存结构化状态和索引：
  ...
  - `makerhub_logs`：业务日志运行期主数据；旧 `/app/config/logs/*.log` 仅作为迁移输入。
  - `archive_model_index`：模型库卡片索引，用于减少全量扫描归档目录。
  - `makerhub_metadata`：迁移标记、索引状态等元信息。
- 文件系统存模型本体和导入入口：
  - `/app/data`：MakerWorld 归档模型、本地模型最终库、图片、模型文件、附件和历史 `meta.json`。
  - `/app/data/local`：本地整理入口、导入候选、整理中和输出目录。
  - `/app/config/config`：运行配置和旧部署配置文件迁移来源；旧 `/app/config/config.json` 兼容作为迁移输入。
  - `/app/config/state`：旧部署 JSON 状态迁移来源、上传暂存、预览队列 marker、合并备份、必要的进程锁和临时文件。
  - `/app/config/logs`：旧部署历史日志迁移来源；新业务日志写入 Postgres。

数据库版本运行期以 Postgres 为结构化状态和业务日志源；旧 JSON/marker/log 文件只作为迁移输入，不作为数据库不可用时的运行期后备。

运行状态字段、状态枚举、事件 scope 和写入频率约束见 [Runtime State Contracts](modules/state_contracts.md)。涉及 `archive_queue`、`missing_3mf`、`organize_tasks`、`subscriptions_state`、`remote_refresh_state` 等 JSON state 的改动，应先更新该契约，再修改写入端和前端消费者。
...
“统领全项目”的会话负责边界、compose、数据库迁移、版本/README/发布；普通功能会话尽量只在一个模块内完成。
