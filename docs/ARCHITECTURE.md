...
- Postgres 存结构化状态和索引：
  ...
  - `makerhub_logs`：业务日志运行期主数据；旧 `/app/logs/*.log` 仅作为迁移输入。
  - `archive_model_index`：模型库卡片索引，用于减少全量扫描归档目录。
  - `makerhub_metadata`：迁移标记、索引状态等元信息。
- 文件系统存模型本体和导入入口：
  - `/app/archive`：MakerWorld 归档模型、本地模型最终库、图片、模型文件、附件和 `meta.json`。
  - `/app/local`：本地整理入口、导入候选、整理中和输出目录。
  - `/app/config`：旧部署配置文件迁移来源。
  - `/app/state`：旧部署 JSON 状态迁移来源，以及必要的进程锁/临时文件。
  - `/app/logs`：旧部署历史日志迁移来源。
