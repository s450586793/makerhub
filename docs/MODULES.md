...
| 归档 / MakerWorld / 下载 / 3MF | [archive.md](modules/archive.md) | 单模型归档、作者/收藏夹/合集批量发现、CloakBrowser 控制面与静态资源流式下载、3MF 下载、缺失 3MF | `archive_worker.py`, `process_jobs.py`, `makerworld_pipeline/*`, `makerworld_parsers/*`, `makerworld_browser_client.py`, `asset_downloader.py`, `legacy_archiver.py`（兼容）, `batch_discovery.py`（兼容）, `three_mf.py` | `test_makerworld_transport_boundary.py`, `test_batch_discovery.py`, `test_missing_3mf.py`, `test_three_mf_quota.py` |
| 订阅 / 来源库 | [subscriptions.md](modules/subscriptions.md) | 订阅 CRUD、定时同步、关注作者/默认收藏夹/关注合集导入、来源卡、来源快照 | `subscriptions.py`, `source_library.py`, `makerworld_pipeline/*`, `makerworld_parsers/*`, `source_health.py`, subscription pages | `test_subscriptions.py`, `test_source_library.py`, `test_source_health.py` |
...
- 改 `archive_queue`、`missing_3mf`、`organize_tasks`、`subscriptions_state`、`remote_refresh_state`、`source_refresh_queue`、`source_refresh_runs`、`runtime_runs`、`runtime_batches`、`runtime_failures` 等状态字段前，先看 [Runtime State Contracts](modules/state_contracts.md)。
- 本地导入、移动端导入、local 文件夹监听都应复用本地整理/导入链路；不要再写一套 zip/STL/3MF 分类逻辑。
- 归档和订阅批量发现通过 `makerworld_pipeline/` 与 `makerworld_parsers/` 实现；`batch_discovery.py` 仅提供兼容 re-export，MakerWorld 控制面变化应优先修 Pipeline 或 BrowserTransport。
- `advanced.scraping_engine` 在 `v0.17.0` 仅用于兼容旧 JSON 和旧客户端请求；运行时固定走 BrowserTransport，前端不得继续发送该字段。
