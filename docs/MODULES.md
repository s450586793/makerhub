...
| 归档 / MakerWorld / 下载 / 3MF | [archive.md](modules/archive.md) | 单模型归档、作者/收藏夹/合集批量发现、双通道控制面与静态下载、3MF 下载、缺失 3MF | `archive_worker.py`, `process_jobs.py`, `makerworld_pipeline/*`, `makerworld_parsers/*`, `makerworld_browser_client.py`, `asset_downloader.py`, `legacy_archiver.py`（兼容）, `batch_discovery.py`（兼容）, `three_mf.py` | `test_makerworld_transport_boundary.py`, `test_batch_discovery.py`, `test_missing_3mf.py`, `test_three_mf_quota.py` |
