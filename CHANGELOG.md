...
- 3MF 子任务与缺失补下载直接复用本地归档资料，只处理选定或缺失的配置文件，不再重新抓取模型资料、图片、附件与评论，也不重复整理资源目录；进度仅展示 3MF 与落盘索引。
- 优先重试已保存的文件直链，网络超时、限流和服务端临时错误保留原地址；直链被拒绝或失效时，每轮最多重新授权一次，新链接在传输前保存，避免中断后重复授权。没有直链时才唤醒浏览器打开模型页申请授权。
- 浏览器识别持续的 Cloudflare 安全验证页，明确提示对应站点需要验证并暂停补下载，不再归为泛化的 HTTP 错误或反复同步 Cookie；短暂可自行通过的检查保留等待时间。

## 2026-09-25 · v0.20.10

- 设置页标题与右上角账号头像合并到同一顶部栏，减少重复标题区域和留白。
- 窄屏标题在自身区域内换行，保留导航按钮、账号菜单和深浅色主题，系统更新与日志继续按宽度自适应排列。

## 2026-09-24 · v0.20.9

- 设置页的“容器更新”和“更新日志”按可用宽度自适应排列，宽屏左右并排，手机或窄窗口上下显示。
- 更新操作和状态提示保留在更新区域，设置标签、长容器名、命令和日志内容可在窄屏内换行，保持深浅色主题兼容。

## 2026-09-23 · v0.20.8

- 3MF 下载定位排除搜索标签、普通导航链接及其内部控件，修复国区把“无 AMS 3D打印模型下载”标签当成下载按钮，跳转搜索页后持续等待授权的问题。
- 下载控件查找和坐标读取保持在同一浏览器脚本上下文，避免 Puppeteer 迁移元素句柄时的 JavaScript world 错误；点击前句柄失效最多重新查找两次，遮挡或禁用时不点击。
- 3MF 授权只对明确的连接初始化故障重试；点击后授权超时、断连或结果不确定时不再整轮重复点击，响应监听已过期时也不继续点击。保留现有下载并发、账号 Gate、每日限额及众筹跳过逻辑。

## 2026-09-22 · v0.20.7

- 从 MakerHub 手动打开指纹浏览器后，独立保存该平台的手动使用时间；后台抓取不再覆盖默认 30 分钟的保留期，避免窗口被误按 2 分钟空闲规则关闭。国区和国际站独立判断，保留原有空闲回收设置。
- 手动打开允许重新尝试启动处于自动恢复冷却中的 profile；启动失败仍只尝试一次并保留冷却，后台自动启动继续遵守重试限制。
- 浏览器已经运行时先尝试连接，连接成功后清除过期恢复状态；冷却只阻止连接失败后的重复自动重启，不再提前拒绝正常的登录态同步。

## 2026-09-22 · v0.20.6

- 模型库后台归档完成后只提示“有更新”，不再自动替换正在浏览的卡片；点击刷新后才回到最新的第一页，失败时保留原有列表。
- 返回详情前的列表或浏览器后退时保留已加载范围、顺序和定位；追加分页同步地址栏页码，避免累计多页被最后一页或旧缓存覆盖。
- 返回模型库仍只校验当前一页；即使总数不变但排序已变化，也延后到手动刷新应用，不增加全历史分页请求。

## 2026-09-22 · v0.20.5

- 账号入口由侧栏卡片移至右上角圆形头像，按 MakerWorld 风格向下展开菜单，统一收纳个人信息、系统设置、主题、当前和最新版本及退出登录。
- 模型详情的搜索框与账号头像保持同排，保留本地模型搜索；支持窄屏、深浅色主题、键盘打开与 Escape 关闭，点击外部或切换页面自动收起菜单。

## 2026-09-22 · v0.20.4

- 修复国际站 3MF 下载误点模型说明中的 Download 跳转链接：排除正文、新窗口跳转、模型导航及其他文件类型，避免重复打开模型页并等待授权超时。
- 识别众筹模型数据和浏览器中的众筹项目入口，跳过 3MF 下载，继续归档标题、图片、说明、评论、附件及配置资料；资料归档完成即结束，不再创建 3MF 子任务或待补记录。
- 已有众筹待补在下次处理后自动移出待补列表；刷新与目录整理不会恢复旧下载地址或再次补下载，保留已有本地文件。跳过 3MF 不作为账号验证成功的依据。

## 2026-09-22 · v0.20.3

- 3MF 授权在取得浏览器跨进程锁后按账号共享请求节奏，默认上次授权结束后随机间隔 5～10 秒；国区和国际站独立计时，浏览器点击与 API 授权共用控制，消除并发任务各自等待后集中请求的问题。
- HTTP 429 按账号逐步冷却并遵守 `Retry-After`；明确的验证码和每日额度响应立即更新现有账号 Gate，排队任务在授权前重新检查。“已验证”继续复用原恢复流程，普通页面、网络错误和已成功的授权不会误关闭 Gate。
- 3MF 文件传输遇到超时、断网或服务器临时错误时，最多复用原签名地址重试两次，并共享原有传输时长预算；失效地址不盲目重试，移除文件下载前的重复随机等待。
- 保留现有归档并发与每日额度配置，不修改历史归档数据；等待时间继续兼容 `MAKERHUB_THREE_MF_DOWNLOAD_WAIT_MIN_SECONDS` / `MAKERHUB_THREE_MF_DOWNLOAD_WAIT_MAX_SECONDS`。

## 2026-09-21 · v0.20.2

- 修复订阅来源后台刷新时用最后一页覆盖全部已加载卡片的问题；保留已加载范围，正确处理末页不足一页、空末页和重复卡片，每次仍只请求当前页。
- 返回订阅来源页时同步恢复列表与地址栏页码；明确指定其他页码时不再复用不匹配的缓存。
- 订阅封面优先采用接口返回的新图片；暂时缺少图片时保留已有预览，避免旧缓存覆盖新封面或刷新后封面消失。

## 2026-09-16 · v0.20.1

- Compose 为 CloakBrowser 的 `/tmp` 配置上限 2 GiB 的 tmpfs，减少 Chromium 共享临时文件落盘；`/data` 登录资料保持持久化。已有部署需同步 Compose 并重建 CB 服务，App/Worker 镜像更新不会自动修改该挂载。
- 可见自动化标签导航前持久记录目标 ID，正常结束后关闭；进程中断或关闭失败后，下次操作继续回收，保留未记录归属的手动页面，不再只按 API 网址判断标签归属。
- Worker 主循环遇到数据库暂时不可用时按 5、10、20、30 秒退避，保留已有队列；恢复后继续处理，停止信号可结束等待，故障日志不依赖数据库连接。

## 2026-09-15 · v0.20.0

- 模型详情按当前 MakerWorld 详情页重新布局：左侧 4:3 图集与说明，右侧打印机筛选、配置卡、耗材摘要和分段下载菜单；支持深浅主题与手机单列布局。
- 图集支持前后切换、键盘和触摸放大浏览；配置浮层支持查看分盘，STL/CAD 文件集中展示，说明、附件和评论支持吸顶导航及返回配置。
- 下载、收藏、打印标记和分享继续操作 MakerHub 本地归档；许可读取已归档字段，相关模型按需分页查询，不调用 MakerWorld 推荐或下载授权接口。
- 修复说明 HTML 中嵌套 SVG 等移除节点导致详情读取失败的问题；3D 预览按真实文件名选择解析器，静止时不再持续渲染，关闭后释放资源。

## 2026-09-14 · v0.19.0

- 作者、合集、收藏夹、本地及状态分组详情改为数据库筛选分页，保留收藏夹源端顺序和已删除模型语义；分组标签按需搜索，本地合并建议按修订缓存，并修复分组页重置按钮没有清空筛选的问题。
- 来源封面刷新先限定目标分组，每组读取最多 4 张预览；预览没有变化时复用原图，避免为一个来源构造全库模型快照。
- 归档任务摘要由一次 SQL 返回计数、少量任务及验证状态；模型与来源缓存使用数据库修订号，修复缓存签名经过 JSON 保存后被误判过期的问题。
- 本地预览使用持久待办队列，导入和重试直接入队，完成后确认，进程中断后恢复；保留 15 分钟低频扫描找回遗漏任务。
- 为慢速或失败的 CB 操作记录连接、执行、清理和进程总耗时，不记录 Cookie、Token 或请求地址；保留现有浏览器生命周期和正式下载并发配置。

## 2026-09-14 · v0.18.0

- 模型库标签改为打开筛选器后按需搜索，每次最多返回 50 个候选；候选按索引和标记修订缓存，模型分页不再重复传输全库标签。
- 本地整理队列无变化时不再重写状态和发布事件；本地库的进度事件只刷新整理任务，模型完成或来源变化时才刷新卡片。
- 本地库卡片由 PostgreSQL 计算分组统计，每组仅返回 4 张预览；归档和订阅判重按候选 ID、URL 查询索引字段，避免读取全量模型 JSON。
- 源端刷新和订阅发现共用后台准入槽，在启动隔离进程前排队；同站点浏览器 GET 的临时故障共享有上限的退避，成功后清除，不增加请求重试次数。正式归档和 3MF 下载并发、每日限额保持原配置。
- 增加 PostgreSQL 实际查询回归，覆盖分组计数、删除来源、预览顺序、标签搜索和候选判重；无需数据库迁移或修改历史归档数据。

## 2026-09-12 · v0.17.2

- 缺失 `3MF` 已存在 `http_error` 等失败状态时，归档 Pipeline 会废弃历史 `downloadUrl` 并重新获取授权地址，避免“重新下载”反复请求过期或已失效的直链。
- 静态资源下载异常新增结构化 `status_code` 与 `failure_kind`，支持区分 HTTP、超时、连接和重定向故障；日志仍只保留去除 query、fragment 和 URL 凭证后的地址。
- 缺失 `3MF` 列表的表格单元格样式改为仅作用于直接子元素，操作列增加稳定宽度和按钮换行约束，修复窄屏下按钮重叠。
