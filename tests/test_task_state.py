...
class ArchiveQueueStateTest(unittest.TestCase):
    ...
    def test_update_active_task_refreshes_runtime_lease_fields(self):
        state = {
            "archive_queue": {
                "active": [
                    {
        ...
        with patch("app.services.task_state.load_database_json_state", side_effect=lambda key, default: dict(state.get(key) or default)), \
                ...
                patch("app.services.task_state.lease_expiry_from_now", return_value="2026-06-04T10:35:00+08:00"):
            queue = store.update_active_task("task-1", progress=50, message="处理中")

        task = queue["active"][0]
        self.assertEqual(task["progress"], 50)
        self.assertEqual(task["lease_expires_at"], "2026-06-04T10:35:00+08:00")
        self.assertEqual(task["heartbeat_at"], "2026-06-04T10:05:00+08:00")
        self.assertEqual(task["started_at"], "2026-06-04T10:00:00+08:00")
    ...
    def test_repair_archive_queue_refreshes_recent_progress_with_expired_lease(self):
        state = {
            "archive_queue": {
                "active": [
                    {
    ...
    def test_resume_verification_paused_archive_tasks_recovers_legacy_browser_session_message(self):
        ...
        self.assertEqual(queue["resumed_count"], 2)
        self.assertEqual(
            [item["id"] for item in queue["resumed_items"]],
            ["legacy-browser-session", "legacy-download-probe"],
        )
        self.assertEqual(queue["queued"][0]["status"], "queued")
        self.assertTrue(queue["queued"][0]["meta"]["browser_session_recovery"])
        self.assertEqual(queue["queued"][1]["status"], "queued")
        self.assertTrue(queue["queued"][1]["meta"]["browser_session_recovery"])
        self.assertEqual(queue["queued"][2]["status"], "paused")
